...
export class StrataDateField extends StrataElement {
  @property({type:String}) value=""; @property({type:String}) label="Date"; @property({type:Boolean,reflect:true}) disabled=false;
  ...
  render(){return html`<label class="strata-field strata-date-field"><span>${this.label}</span><input type="date" .value=${this.value} ?disabled=${this.disabled} @input=${this.handleInput} @change=${this.handleChange}/></label>`;}
}
