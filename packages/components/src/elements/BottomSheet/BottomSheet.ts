...
export class StrataBottomSheet extends StrataElement {
  ...
  render(){return html`<section class="strata-bottom-sheet" ?hidden=${!this.open} role="dialog" role="dialog" aria-modal="true" aria-label=${this.title}><header><strong>${this.title}</strong><button type="button" aria-label="Close" @click=${()=>this.open=false}><iconify-icon icon="carbon:close"></iconify-icon></button></header><slot></slot></section>`;}
}
