...
export const Semantic: Story = { render: () => `<section class="strata-story-foundation-page"><h1>Monochrome establishes structure.</h1><p>Color establishes meaning. The spectrum is deliberately restrained.</p><div class="strata-story-spectrum-grid">${colors.map(([tone,meaning])=>`<div class="strata-story-spectrum-item"><strata-badge tone="${tone}">${tone}</strata-badge><small class="strata-story-scale-label">${meaning}</small></div>`).join('')}</div></section>` };
