...
function checkFile(file) {
  ...
  lines.forEach((line, i) => {
    ...
    if (radius && !allowedRadius.test(radius[1])) violations.push(`${file}:${n} non-token radius`);
    const border = line.match(/(?:^|[;{])border(?:-(?:width|top|right|bottom|left))?\s*:\s*([^;]+)/);
    if (border && !allowedBorderWidth.test(border[1]) && !border[1].includes('solid')) violations.push(`${file}:${n} non-token border width`);
    if (file.endsWith('NavItem.ts') && line.includes(':host([selected])') && line.slice(line.indexOf(':host([selected])')).includes('border:')) violations.push(`${file}:${n} selected navigation must not use a decorative border`);
    if (line.includes('<iconify-icon') && /icon\s*=\s*["'](?!carbon:)/.test(line)) violations.push(`${file}:${n} icon must use carbon:*`);
  });
}
